...
/**
 * 
 * 功能描述：代码生成器 包括自动生成service和controller 完成后刷新项目
 * 
...
public class MyBatisPlusGenerator {
	...
	public void myBatisPlusGenerator() {
		...
		config.setActiveRecord(false) // 是否支持AR模式
				.setOpen(false).setAuthor("yangm") // 作者
		...
		dsConfig.setDbType(DbType.MYSQL) // 设置数据库类型
				.setDriverName("com.mysql.jdbc.Driver").setUrl("jdbc:mysql://localhost:3306/test").setUsername("root")
				.setPassword("123456");
		...
		stConfig.setCapitalMode(true) // 全局大写命名
				...
				// .setSuperControllerClass("com.baomidou.demo.TestController")//自定义 controller父类
				// .setTablePrefix("tbl_") // 全局表前缀
				.setInclude(tableNames); // 生成的表

		// 4. 包名策略配置
		PackageConfig pkConfig = new PackageConfig();
		pkConfig.setParent("com.github.yangm.mybatisplus").setMapper("dao").setService("service")
